...
void SwapChain::createSwapChain()
{
	...
	if( vkCreateSwapchainKHR( m_device.logical(), &createInfo, nullptr, &m_swap_chain ) != VK_SUCCESS )	
		throw std::runtime_error( "Swap chain creation failed" );

	// Get new swap chain images
	vkGetSwapchainImagesKHR( m_device.logical(), m_swap_chain, &imageCount, nullptr );
	m_images.resize( imageCount );
	vkGetSwapchainImagesKHR( m_device.logical(), m_swap_chain, &imageCount, m_images.data() );
}
...
{
	destroyImageViews();
	vkDestroySwapchainKHR( m_device.logical(), m_swap_chain, nullptr );
}
